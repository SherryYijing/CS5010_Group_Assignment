...
public class AlphaVantageImpl implements AlphaVantage {
  // A price map it will store <Ticker symbol <Specific date <Price type <Price value> > >.
  ...
  /**
   * An instance for other class to call AlphaVantageImpl quickly.
   *
   * @return an instance.
   */
  ...
  @Override
  public URL getURL(String stockSymbol) throws IllegalArgumentException {
    URL url = null;
      /*
    ...
    String apiKey = "1QDFK9O8GUF7DU0R"; // A back-up api key: "6ZESL7619O96BABK";
  ...
  @Override
  public StringBuilder getOutput(String stockSymbol, URL url) throws IllegalArgumentException {
    InputStream in = null;
  ...
  @Override
  public void checkTikerAvailable(String companyName) {
    URL url;
    String apiKey = "1QDFK9O8GUF7DU0R";
    try {
    ...
    StringBuilder output = getOutput(companyName, url);
    String[] value = output.toString().split("\n");
    /*
     If length <= 1 which means we didn't find any information about this stock (first line is
     title, and it doesn't be counted).
     */
    if (value.length <= 1) {
      throw new IllegalArgumentException("No data found for " + companyName + "!");
    }
  }
  ...
  public void getPrices(String stockSymbol) {
    ...
    StringBuilder output = getOutput(stockSymbol, url);
    /*
    ...
    String[] value = output.toString().split("\n");
    if (value.length <= 1) {
      throw new IllegalArgumentException("No data found for " + stockSymbol);
    }
